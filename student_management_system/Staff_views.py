...
from studentmsapp.models import Staff,Staff_Notification,Staff_leave,Staff_Feedback,Subject,Session_Year,Student,Attendance,Attendance_Report, StudentResult
...
def STAFF_APPLY_LEAVE_SAVE(request):
    if request.method == "POST":
        ...
        print(leave_message, leave_date)
        ...
        leave.save()
        messages.success(request,'Application for Leave is Sent')
...
def STAFF_SAVE_ATTENDANCE(request):
    ...
    for student_id in student_id:
        ...
        Attendance_Report.objects.create(
            student_id=student,
            attendance_id=attendance,
            status=1  # Assuming all students are present by default
        )
    return redirect('staff_take_attendance')

def ATTENDANCE_PRESENT(request,id):
    attendance_report = Attendance_Report.objects.get(id=id)
...
def STAFF_SAVE_RESULT(request):
    if request.method == "POST":
        ...
        if check_exists:
            ...
            result.save()
            messages.success(request,' Result Marks are Updated Successfully')
            ...
            result.save()
            messages.success(request,' Result Marks are Added Successfully')
            return redirect('staff_add_result')
