...
from studentmsapp.models import Course,Session_Year,CustomUser,Student,Staff,Subject,Staff_Notification,Staff_leave,Staff_Feedback,Student_Notification,Student_Feedback,Student_leave,Attendance,Attendance_Report
...
@login_required(login_url='/')#to not allow access when logout
def HOME(request):
...
# print(course)    print(session_Year)
    ...
    return render(request,'HOD/add_student.html',context)#diff Hod
...
def STUDENT_FEEDBACK_SAVE(request):#REPLY_STUDENT_FEEDBACK
    if request.method == "POST":
        feedback_id = request.POST.get('feedback_id')
        feedback_reply = request.POST.get('feedback_reply')
...
def STUDENT_SAVE_NOTIFICATION(request):
    if request.method == "POST":
        ...
        messages.success(request,"Student Notification Is Successfully Send ")
...
# def check_attendance(student_attendance, target_date):
